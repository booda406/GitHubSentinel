...
class Notifier:
    ...
    def notify_linkedin_report(self, report):
        if self.slack_webhook_url:
            self.send_slack_message("LinkedIn", report)
        else:
            LOG.warning("Slack Webhook URL 未配置，无法发送 LinkedIn 报告通知")
    ...
    def send_slack_message(self, report_type, report):
        LOG.info(f"准备发送 {report_type} Slack 消息")
        slack_message = self.format_slack_message(report_type, report)
        try:
            ...
            LOG.info(f"{report_type} Slack 消息发送成功！")
        except requests.RequestException as e:
            LOG.error(f"发送 {report_type} Slack 消息失败：{str(e)}")

    def format_slack_message(self, report_type, report):
        slack_message = f"*[{report_type}] 进展简报*\n\n"
...
#     notifier.notify_hn_report("2024-09-01", hn_report)
    ...
    # Test LinkedIn report Slack notification
    linkedin_report = """
...
"""
    notifier.notify_linkedin_report(linkedin_report)
